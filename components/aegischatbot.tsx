...
export default function AegisChatBot() {
  ...
  const generatePDF = async () => {
    if (!reportRef.current || !vulnerabilities.length) return;
    ...
    try {
      ...
      // Create cover page
      // Background rectangle with gradient-like effect
      ...
      pdf.roundedRect(20, 40, 170, 60, 3, 3, 'F');
      
      // Add Report Title
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(24);
      pdf.setTextColor(79, 70, 229); // Indigo
      pdf.text('Security Vulnerability Report', 105, 65, { align: 'center' });
      
      // Add AegisAI branding
      pdf.setFontSize(16);
      pdf.setTextColor(107, 114, 128); // Gray
      pdf.text('Generated by AegisAI', 105, 80, { align: 'center' });
      
      // Add date with normal font
      ...
      pdf.text(`Generated on ${dateStr}`, 105, 90, { align: 'center' });
      
      // Add summary stats boxes
      ...
      pdf.roundedRect(startX, startY, boxWidth, boxHeight, 3, 3, 'F');
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(20);
      pdf.setTextColor(220, 38, 38); // Red
      pdf.text(highSeverity.toString(), startX + boxWidth/2, startY + 20, { align: 'center' });
      pdf.setFontSize(10);
      pdf.text('HIGH SEVERITY', startX + boxWidth/2, startY + 30, { align: 'center' });
      
      // Medium severity box
      pdf.setFillColor(255, 237, 213); // Light orange background
      pdf.roundedRect(startX + boxWidth + boxSpacing, startY, boxWidth, boxHeight, 3, 3, 'F');
      pdf.setFontSize(20);
      pdf.setTextColor(249, 115, 22); // Orange
      pdf.text(mediumSeverity.toString(), startX + boxWidth + boxSpacing + boxWidth/2, startY + 20, { align: 'center' });
      pdf.setFontSize(10);
      pdf.text('MEDIUM SEVERITY', startX + boxWidth + boxSpacing + boxWidth/2, startY + 30, { align: 'center' });
      
      // Low severity box
      pdf.setFillColor(254, 249, 195); // Light yellow background
      pdf.roundedRect(startX + 2 * (boxWidth + boxSpacing), startY, boxWidth, boxHeight, 3, 3, 'F');
      pdf.setFontSize(20);
      pdf.setTextColor(202, 138, 4); // Yellow/amber
      pdf.text(lowSeverity.toString(), startX + 2 * (boxWidth + boxSpacing) + boxWidth/2, startY + 20, { align: 'center' });
      pdf.setFontSize(10);
      pdf.text('LOW SEVERITY', startX + 2 * (boxWidth + boxSpacing) + boxWidth/2, startY + 30, { align: 'center' });
      
      // Add code information box
      pdf.setFillColor(255, 255, 255);
      pdf.roundedRect(20, 180, 170, 70, 3, 3, 'F');
      
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(12);
      pdf.setTextColor(79, 70, 229); // Indigo
      pdf.text('Code Analysis Summary', 30, 195);
      ...
      pdf.text('CONFIDENTIAL - For internal use only', 105, 280, { align: 'center' });
      
      // Start report content on the second page
      pdf.addPage();
      
      // Add title and header for the content pages
      const reportTitle = 'Security Vulnerability Report';
      pdf.setFontSize(16);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(79, 70, 229); // Indigo
      ...
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(107, 114, 128); // Gray
      pdf.text(`Generated on ${dateStr}`, 14, 22);
      ...
      pdf.text('Summary', 14, contentYPos);
      ...
      summaryText.forEach(text => {
        pdf.text(text, 14, contentYPos);
        contentYPos += 6;
      });
      
      // Add vulnerability details section
      let yPos = contentYPos + 6;
      pdf.setFontSize(14);
      pdf.setTextColor(33, 33, 33);
      pdf.text('Vulnerability Details', 14, yPos);
      yPos += 6;
      
      // Sorting vulnerabilities for the report by severity and CVSS score
      const reportVulnerabilities = [...vulnerabilities].sort((a, b) => {
        ...
        if (aValue !== bValue) {
          return bValue - aValue; // First sort by severity (high to low)
        }
        
        // Then by CVSS score if severity is the same
      ...
      // Function to add a page if we're near the bottom
      const checkAndAddPage = (requiredSpace: number = 20) => {
        if (yPos > 270 - requiredSpace) { // A4 height is 297mm, leave some margin
          pdf.addPage();
          yPos = 15;
          return true;
        }
      ...
      reportVulnerabilities.forEach((vulnerability, index) => {
        checkAndAddPage(60); // Check if we need a new page for this vulnerability
        ...
        // Draw severity colored marker
        pdf.setFillColor(getSeverityHexColor(vulnerability.severity));
        pdf.rect(14, yPos, 3, 3, 'F');
      ...
      if (secureCode) {
        ...
        pdf.setFontSize(14);
        pdf.setTextColor(33, 33, 33);
        pdf.text('Secure Code', 14, yPos);
        yPos += 6;
        
        pdf.setFontSize(9);
        pdf.setTextColor(60, 60, 60);
        
        const codeLines = secureCode.split('\n');
      ...
      // Add diff view section if it exists
      if (diff && diff.length > 0) {
        checkAndAddPage();
        yPos += 8;
        pdf.setFontSize(14);
        pdf.setTextColor(33, 33, 33);
        pdf.text('Code Changes', 14, yPos);
        yPos += 6;
        
        pdf.setFontSize(9);
        
        // Helper function to get color for change type
        const getDiffColor = (changeType: string) => {
          switch (changeType) {
        ...
        pdf.text('Line', 14, yPos);
        pdf.text('Original Code', 30, yPos);
        pdf.text('Secure Code', 105, yPos);
        yPos += 3;
        
        // Simple line separator
        pdf.setDrawColor(200, 200, 200);
        pdf.line(14, yPos, 190, yPos);
        yPos += 4;
        
        // Draw each line of the diff
        diff.forEach(diffLine => {
          checkAndAddPage(10);
          
          const { lineNumber, originalLine, securedLine, changeType } = diffLine;
          
          // Line number
          pdf.setFontSize(8);
          pdf.setTextColor(100, 100, 100);
          pdf.text(lineNumber.toString(), 14, yPos);
          
          // Set color based on change type
          pdf.setTextColor(getDiffColor(changeType));
          
          // Original line (left side)
          const truncatedOriginal = originalLine.substring(0, 50) + (originalLine.length > 50 ? '...' : '');
          pdf.text(truncatedOriginal, 30, yPos);
          
          // Secured line (right side)
          const truncatedSecured = securedLine.substring(0, 50) + (securedLine.length > 50 ? '...' : '');
          pdf.text(truncatedSecured, 105, yPos);
          
          // Add change marker
          if (changeType !== 'unchanged') {
            const marker = changeType === 'added' ? '+' : changeType === 'removed' ? '-' : '~';
            pdf.setFontSize(10);
            pdf.text(marker, 26, yPos);
          }
  ...
  return (
    <div className="max-w-6xl w-full mx-auto relative z-10 flex items-center space-x-4 rounded-sm flex-col bg-zinc-950/50 p-10 ring-1 ring-white/10 backdrop-blur-md">
      ...
      {vulnerabilities.length > 0 && (
        <div className="w-full mb-6" ref={reportRef}>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-500 hover:bg-indigo-600 text-white text-xs rounded-md transition-colors"
              >
                {isGeneratingPDF ? (
                  <>
                  ...
                  <>
                    <IconFileDownload className="w-3.5 h-3.5" />
